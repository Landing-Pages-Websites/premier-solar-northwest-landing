...
/**
 * useMegaLeadForm — Premier Solar NW HVAC LP form submission hook.
 * CUSTOMER_ID + SITE_ID are real values provisioned via
 * `mega site-tracking enable` for https://book.premiersolarnw.com.
 */

const DEFAULTS = {
  CUSTOMER_ID: "0e9e2686-9b2b-4bf4-b4fe-903e65ceb028", // Premier Solar Northwest
  SITE_ID: "ac6c1dfd-a0e9-48b6-a6c5-5d67ebbbfab6",
...
export function useMegaLeadForm(
  ...
  const submit = useCallback(
    async (formData: Record<string, unknown>): Promise<SubmissionResponse> => {
      ...
      // PSN HVAC LP uses firstName + lastName as separate keys per task spec.
      if (!formData.email) {
        throw new Error("email is required");
      }
      if (!formData.firstName && !formData.name && !formData.fullName) {
      ...
      const payload: SubmissionPayload = {
        ...
        source_provider: options.sourceProvider ?? "premier-solar-northwest-landing",
        form_data: formData,
